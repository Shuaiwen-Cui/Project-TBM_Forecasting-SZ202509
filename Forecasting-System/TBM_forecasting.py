...
def update_visualization():
    ...
    fig_new.update_layout(
        title=f'{feature_names[current_feature]} (特征{current_feature}) - 时间: {datetime.now().strftime("%H:%M:%S")}<br>显示整个显存数据 ({actual_data_count}/{valid_length} 个有效数据点)',
            ...
            dict(
                ...
                buttons=list([
                    dict(
                        ...
                        args=[
                            {
                                "title": f"{name} (特征{i}) - 时间: {datetime.now().strftime('%H:%M:%S')}<br>显示整个显存数据 ({actual_data_count}/{valid_length} 个有效数据点)"
                            },
                            {
