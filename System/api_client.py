...
class TBMAPIClient:
    ...
    def get_latest_features(self):
        """
        获取最新的31个特征数据 - 获取过去1分钟的数据，选择ID最大的那一条
        ...
        print(f"📊 获取到 {valid_count}/31 个有效特征值 (记录ID: {latest_record.get('id', 'N/A')})")
    ...
    def get_latest_features_with_timestamp(self):
        """
        获取最新的31个特征数据和时间戳
        
        Returns:
            tuple: (feature_values, timestamp_info)
                - feature_values: 31个特征值数组
                - timestamp_info: 包含时间戳信息的字典
        """
        # 获取最新数据
        ...
        timestamp_info = {
            ...
            'origin_time': parsed_data.get('origin_time'),
            'my_time': parsed_data.get('my_time'),
        ...
        return feature_values, timestamp_info
    
    def get_historical_data(self, minutes_back=5):
        """
        获取历史数据用于初始化缓冲区
        
        Args:
            minutes_back (int): 获取多少分钟前的数据
            
        Returns:
            list: 包含历史数据记录的列表，每个记录包含31个特征值
        """
        try:
            print(f"📅 获取历史数据: 最近{minutes_back}分钟")
            
            # 获取历史数据（不指定时间范围，让API返回最近的数据）
            # 由于每分钟可能有2条数据，所以获取更多数据确保覆盖足够的时间范围
            api_response = self.fetch_data_by_time_range(
                begin_time=None,
                end_time=None,
                limit=minutes_back * 3  # 获取更多数据，确保覆盖足够的时间范围
            )
            ...
            for i, record in enumerate(records):
                ...
                if parsed_data:
                    ...
                    print(f"   记录 {i+1}: ID={parsed_data.get('id')}, 有效特征={valid_count}/31")
            
            # 按时间分组，每分钟只取ID最大的那一条（靠后的）
            time_groups = {}
            for record in historical_data:
                timestamp = record['timestamp']
                if timestamp in time_groups:
                    # 如果同一分钟有多条记录，保留ID较大的
                    if int(record['record_id']) > int(time_groups[timestamp]['record_id']):
                        time_groups[timestamp] = record
                else:
                    time_groups[timestamp] = record
            
            # 按时间排序
            sorted_data = sorted(time_groups.values(), key=lambda x: x['timestamp'])
            
            print(f"✅ 成功解析 {len(sorted_data)} 条历史记录（已去重，每分钟1条）")
            return sorted_data
